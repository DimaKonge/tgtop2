...
VPS rollout preparation on 2026-09-03: `/opt/tgtop` is on `main` at `242fcb6`, has no configured Git remote, and contains a large pre-existing dirty tree with local release backups. The reviewed `scripts/release-vps.sh` was run locally in `DRY_RUN=1` using the production GitHub checkout; the full test/build/package preflight passed and produced archive SHA-256 `7a867389bbf63fd16036d4d2a4c7db68514debbcf3463fdb51748ec1ff33eef5`. Dry-run uploaded nothing and changed no VPS files. The active live static bundle remains the older served build until the owner activates the prepared release through the established VPS release process.
