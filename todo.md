...
- [ ] Привязать owner-only личный диалог TG TOP Assistant к аккаунту @dimij через постоянный Telegram numeric ID, полученный только из первого входящего сообщения, с запретом обработки всех остальных личных чатов.
...
- [ ] Отправить единственное стартовое приветствие «TG TOP Assistant готов» только после server-side привязки постоянного Telegram ID @dimij; не разрешать иные исходящие DM до запуска owner-only responder и audit-правил.
