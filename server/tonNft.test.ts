...
import { areTonAddressesEqual, classifyWalletNft, isTonWalletAddress, normalizeWalletNft } from "./tonNft";
...
describe("wallet NFT classification", () => {
  ...
  it("correctly matches bounceable, non-bounceable and raw TON addresses", () => {
    const eq = "EQCD39VS5jcptHL8vMjEXrzGaRcCVYto7HUn4bpAOg8xqB2N";
    const uq = "UQCD39VS5jcptHL8vMjEXrzGaRcCVYto7HUn4bpAOg8xqEBI";
    expect(areTonAddressesEqual(eq, uq)).toBe(true);
    expect(areTonAddressesEqual(eq, eq)).toBe(true);
    expect(areTonAddressesEqual(eq, "EQAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAM9c")).toBe(false);
    expect(areTonAddressesEqual(undefined, eq)).toBe(false);
  });
});
