...
describe("private log destination commands", () => {
  it("accepts only exact commands for the supported private topics", () => {
    ...
    expect(parsePrivateLogDestinationCommand("/tgtop_log_additions@TGTOP_robot")).toBe("additions");
  ...
  it("routes support setup to the main bot and all service logs to the reserve bot", () => {
    ...
    expect(source).toContain('/^\\/tgtop_log_additions(?:@\\w+)?$/');
