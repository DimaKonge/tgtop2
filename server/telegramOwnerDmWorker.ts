...
export async function runTelegramOwnerDmWorker(workerId: string) {
  ...
  const handleIncoming = async (event: NewMessageEvent) => {
    if (!event.isPrivate || String(event.message.senderId) !== binding.ownerTelegramId) return;
    const text = event.message.text?.trim();
    if (!text || text.length > OWNER_DM_MAX_INPUT_CHARS) return;
    const created = await enqueueTelegramOwnerDmJob({ telegramMessageId: String(event.message.id), ownerTelegramId: binding.ownerTelegramId, encryptedInput: encryptPayload(text) });
    if (created) await recordTelegramUserAgentAuditEvent({ action: "owner_dm_received", actorOpenId: "telegram-owner-worker", details: `message=${event.message.id}` });
  };
  client.addEventHandler(event => { void handleIncoming(event); }, new NewMessage({ incoming: true, fromUsers: [binding.ownerTelegramId], forwards: false }));
