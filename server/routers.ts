...
import { getWalletNfts, resolveNftItemByAddress, verifyTonNftOwnership } from "./tonNft";
...
export const appRouter = router({
  ...
  tgTop: router({
    ...
    resolveTonNft: protectedProcedure
      .input(z.object({ addressOrName: z.string().trim().min(1).max(128) }))
      .query(async ({ input }) => await resolveNftItemByAddress(input.addressOrName)),
    ...
    createNft: protectedProcedure
      ...
      .mutation(async ({ ctx, input }) => {
        ...
        if (input.assetClass === "onchain" && input.nftItemAddress) {
          isVerifiedOnchain = await verifyTonNftOwnership(input.nftItemAddress, input.ownerWalletAddress);
        }
        if (!canCreateNftListing(input.assetClass, isVerifiedOnchain)) {
          throw new Error("Публикация On-chain NFT доступна только для подтвержденных активов вашего подключенного TON-кошелька");
        }
        await db.createNftListing({
          ...input,
          ownerOpenId: ctx.user.openId,
          ownerUsername: ctx.user.name || ctx.user.openId.slice(0, 8),
          ownershipVerifiedAt: input.assetClass === "offchain" || isVerifiedOnchain ? new Date() : null,
          ownershipVerification: input.assetClass === "offchain" ? "tg-top-internal" : (isVerifiedOnchain ? "tonapi-wallet-verified" : null),
          status: "available",
        });
        return { success: true };
      }),
