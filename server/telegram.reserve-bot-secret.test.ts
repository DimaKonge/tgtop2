...
describe("Telegram reserve bot secret", () => {
  const liveIt = process.env.RUN_LIVE_TELEGRAM_TESTS === "true" ? it : it.skip;

  liveIt("authenticates the reserve bot without exposing its token", async () => {
    const token = process.env.TELEGRAM_RESERVE_BOT_TOKEN;
    expect(token, "TELEGRAM_RESERVE_BOT_TOKEN must be configured").toBeTruthy();
