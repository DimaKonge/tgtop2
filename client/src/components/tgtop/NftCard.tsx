        ...
        <div className="flex items-center gap-1.5">
          {(nft.listingType === "sale" || nft.listingType === "both") && nft.status === "available" && onBuy && (
            <button
              type="button"
              onClick={() => onBuy(nft)}
              className="rounded-lg bg-[#3f8cff] px-2.5 py-1.5 text-[10px] font-semibold text-white transition-colors hover:bg-[#3478db] active:scale-[0.98]"
            >
              {language === "en" ? "Buy via Escrow" : "Купить"}
            </button>
          )}
