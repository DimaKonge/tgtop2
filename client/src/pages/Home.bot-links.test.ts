...
describe("TG TOP production bot links", () => {
  ...
  it("uses the shared GroupCard component for ranked and general catalog placements", () => {
    ...
    expect(source).toMatch(/<ToggleGroup\s+type="single"\s+value=\{globalDirection\}/);
    expect(source).toContain('value="Каналы"');
    expect(source).toContain('value="Чаты"');
    expect(source).toContain('value="NFT"');
    ...
    expect(source).toContain('{n(globalCount)} {globalDirection === "NFT" ? "NFT" : ui.groups}');
