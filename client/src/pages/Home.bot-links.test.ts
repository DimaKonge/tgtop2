...
describe("TG TOP production bot links", () => {
  ...
  it("uses the shared GroupCard component for ranked and general catalog placements", () => {
    ...
    expect(source).toContain('const detailSalePriceUnits = parseGramInput(salePriceTon);');
    expect(source).toContain('const detailSaleEnabled = Boolean(isListingForSale && (detailSalePriceUnits ?? 0) > 0);');
    ...
    expect(source).toContain('detailSaleEnabled ? (ownsDetail ? "На продаже" : "Купить за") : "Покупка"');
    ...
    expect(source).toContain('<b className="mt-0.5 text-[11px] leading-3">Перейти</b>');
    ...
    expect(source).toContain('const detailWillDrop = Boolean(ownsDetail && selectedSlot && detailRankingPreviewSlotNumber && detailRankingPreviewSlotNumber > selectedSlot.slotNumber);');
    expect(source).toContain('Группа опустится на ${detailRankingPreviewSlotNumber}-ю позицию');
    expect(source).toContain('{ownsDetail ? "Обновить лот" : "Перебить лот"} · #{selectedSlot.slotNumber}');
