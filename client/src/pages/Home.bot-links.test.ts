...
describe("TG TOP production bot links", () => {
  ...
  it("uses the shared GroupCard component for ranked and general catalog placements", () => {
    ...
    expect(source).toContain('detailSaleEnabled ? "grid-cols-3" : "grid-cols-2"');
    ...
    expect(source).toContain('<span className="mt-1 block text-center text-[10px] font-extrabold leading-none tracking-[0.02em] text-[#63f5b1]">+{formatGram(detailEntryReward)} GRAM</span>');
    ...
    expect(source).toContain('min-h-[58px]');
    expect(source).not.toContain('min-h-[64px]');
    ...
    expect(source).toContain('<span className="flex items-center justify-center gap-1.5"><Send className="h-3.5 w-3.5 text-white/90" /><b className="text-[11px] leading-3">Перейти</b></span>');
    expect(source).toContain('onClick={() => { if (detailEntryUrl) openTelegramCommunityLink(detailEntryUrl); }}');
    ...
    expect(source).toContain('<b className="text-[11px] leading-3">Перейти</b>');
