...
function GroupCard({ group, variant = "list", onClick }: { group?: Group | null; variant?: GroupCardVariant; onClick: () => void }) {
  ...
  const cardStyle = lead
    ? "min-h-[42vh] border-[#3f8cff]/35 bg-[radial-gradient(circle_at_85%_12%,rgba(63,140,255,0.18),transparent_34%),#141c27] p-6"
    : variant === "secondary"
  ...
  return <button onClick={onClick} className={`w-full rounded-2xl border text-left transition-colors hover:border-[#3f8cff]/45 active:scale-[0.99] ${cardStyle}`}>
    {group ? <span className={shellStyle}><Avatar group={group} large={lead} compact={compact} /><span className={compact ? "w-full min-w-0" : "min-w-0 flex-1"}><b className={`${lead ? "text-xl" : compact ? "text-[11px]" : "text-sm"} block truncate font-semibold text-slate-100`}>{group.title}</b><small className={`mt-1 block truncate text-xs text-slate-500 ${compact ? "hidden" : ""}`}>{group.username ? `@${group.username}` : group.category} · {n(group.membersCount)} участников</small><small className={`mt-1 block text-[10px] font-medium tracking-wide text-[#72a8ff] ${compact ? "hidden" : ""}`}>ПРОВЕРЕНА TG TOP</small></span>{!compact && <ChevronRight className="h-4 w-4 shrink-0 text-slate-600" />}</span> : <span className={shellStyle}><span className={`${lead ? "h-16 w-16" : compact ? "h-9 w-9" : "h-11 w-11"} grid shrink-0 place-items-center rounded-xl border border-dashed border-white/20 text-slate-500`}><Plus className="h-4 w-4" /></span><span className={compact ? "w-full" : ""}><b className={`${compact ? "text-[11px]" : "text-sm"} block font-medium text-slate-300`}>{compact ? "Добавить" : "Добавить группу"}</b><small className={`mt-1 block text-[10px] text-slate-500 ${compact ? "hidden" : ""}`}>Из личной папки</small></span></span>}
  </button>;
}
...
export default function Home() {
  ...
  return <div className="tg-shell min-h-screen bg-[#0b0f14] text-slate-100">
    ...
    <main className="mx-auto max-w-3xl px-4 pb-28 pt-5">
      {page === "top" && <section className="space-y-4"><div className="flex items-end justify-between"><div><p className="text-xs font-medium uppercase tracking-[0.16em] text-[#72a8ff]">Каталог</p><h1 className="mt-1 text-2xl font-semibold">Площадки</h1><p className="mt-1 text-sm text-slate-500">Проверенные сообщества и открытая видимость.</p></div><Button onClick={() => setFiltersOpen(true)} variant="outline" className="border-white/10 bg-[#111720] text-slate-200"><Filter className="mr-2 h-4 w-4" />Фильтр</Button></div><div className="space-y-3"><GroupCard group={leadSlot.group} variant="lead" onClick={() => leadSlot.group ? openGroup(leadSlot.group.id) : openMine(leadSlot)} /><div className="grid grid-cols-2 gap-3">{secondTier.map(slot => <GroupCard key={slot.slotNumber} group={slot.group} variant="secondary" onClick={() => slot.group ? openGroup(slot.group.id) : openMine(slot)} />)}</div><div className="grid grid-cols-4 gap-2">{thirdTier.map(slot => <GroupCard key={slot.slotNumber} group={slot.group} variant="compact" onClick={() => slot.group ? openGroup(slot.group.id) : openMine(slot)} />)}</div></div><section className="pt-2"><div className="mb-3"><h2 className="text-sm font-semibold">Все площадки</h2><p className="mt-0.5 text-xs text-slate-500">Порядок выбранного фильтра</p></div><div className="space-y-2">{generalList.map(group => <GroupCard key={group.id} group={group} variant="list" onClick={() => openGroup(group.id)} />)}{generalList.length === 0 && <div className="rounded-2xl border border-dashed border-white/12 bg-[#111720] p-6 text-center"><p className="text-sm font-medium text-slate-300">В TG TOP пока нет площадок</p><p className="mt-1 text-xs text-slate-500">Добавьте первую группу через личную папку.</p></div>}</div></section></section>}
