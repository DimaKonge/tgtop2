...
type TelegramAnalyticsPoint = { at: number; value: number };
type TelegramAnalyticsGraph = { series: Array<{ key: string; label: string; points: TelegramAnalyticsPoint[] }> };

function TelegramAnalyticsChart({ title, graph, range, accent = "#72a8ff" }: { title: string; graph?: TelegramAnalyticsGraph; range: "day" | "month" | "all"; accent?: string }) {
  const now = Date.now();
  const cutoff = range === "day" ? now - 24 * 60 * 60 * 1_000 : range === "month" ? now - 31 * 24 * 60 * 60 * 1_000 : 0;
  const series = (graph?.series ?? []).map(item => ({ ...item, points: item.points.filter(point => point.at >= cutoff) })).filter(item => item.points.length > 0).slice(0, 3);
...
export default function Home({ onReady }: { onReady?: () => void }) {
  ...
  const telegramHistoricalStats = (telegramHistoricalStatsQuery.data ?? []) as Array<{
    ...
    snapshot: null | { memberCount: number | null; viewsPerPost: number | null; sharesPerPost: number | null; reactionsPerPost: number | null; periodStart: Date | null; periodEnd: Date | null; history: { version?: number; graphs?: Record<string, TelegramAnalyticsGraph>; summary?: Record<string, unknown>; memberHistory?: Array<{ at: number; value: number }>; activityHistory?: Array<{ at: number; value: number }> } };
  }>;
                  ...
                  <SheetContent side="bottom" className="max-h-[88dvh] overflow-y-auto rounded-t-[26px] border-white/10 bg-[#10161f] text-slate-100">
                    ...
                    <div className="space-y-3 px-4 pb-[calc(1.25rem+env(safe-area-inset-bottom))] pt-2">
                      {!telegramUserAgentStatus || telegramUserAgentStatus.status === "disconnected" || telegramUserAgentStatus.status === "error" ? <>
                        ...
                        <div className="space-y-2 rounded-xl border border-violet-300/15 bg-violet-400/[0.045] p-3"><div><b className="block text-xs text-violet-100">История статистики Telegram</b><p className="mt-1 text-[10px] leading-4 text-slate-400">Только разрешённые каналы и официальные агрегаты Telegram. Текст постов, лички и пользователи не загружаются.</p></div><div className="flex gap-2"><Input value={telegramStatsUsername} onChange={event => setTelegramStatsUsername(event.target.value)} autoComplete="off" placeholder="@username канала" className="h-10 border-white/10 bg-[#17212b] text-sm text-slate-100" /><Button onClick={() => allowTelegramHistoricalStats.mutate({ username: telegramStatsUsername })} disabled={allowTelegramHistoricalStats.isPending || telegramStatsUsername.trim().length < 5} className="h-10 shrink-0 bg-violet-500/85 px-3 text-xs text-white">{allowTelegramHistoricalStats.isPending ? ui.loading : "Разрешить"}</Button></div>{telegramHistoricalStats.map(target => <div key={target.id} className="space-y-3 rounded-xl border border-white/8 bg-black/10 p-3"><div className="flex items-center justify-between gap-2"><span className="min-w-0"><b className="block truncate text-xs text-slate-100">{target.title}</b><small className="block text-[10px] text-slate-500">@{target.username} · {target.kind === "channel" ? "канал" : "группа"}</small></span><Button variant="outline" onClick={() => refreshTelegramHistoricalStats.mutate({ username: target.username })} disabled={refreshTelegramHistoricalStats.isPending} className="h-8 shrink-0 border-violet-200/20 px-2 text-[10px] text-violet-100">Обновить</Button></div>{target.snapshot?.history.version === 2 ? <><div className="grid grid-cols-2 gap-2"><Metric label="Подписчики" value={target.snapshot.memberCount === null ? "—" : target.snapshot.memberCount.toLocaleString("ru-RU")} note="текущая официальная цифра" /><Metric label="Охват / пост" value={target.snapshot.viewsPerPost === null ? "—" : target.snapshot.viewsPerPost.toLocaleString("ru-RU")} note="за период Telegram" /><Metric label="Пересылки / пост" value={target.snapshot.sharesPerPost === null ? "—" : target.snapshot.sharesPerPost.toLocaleString("ru-RU")} note="за период Telegram" /><Metric label="Реакции / пост" value={target.snapshot.reactionsPerPost === null ? "—" : target.snapshot.reactionsPerPost.toLocaleString("ru-RU")} note="за период Telegram" /></div><div className="grid grid-cols-3 rounded-lg border border-white/8 bg-[#111720] p-1">{(["day", "month", "all"] as const).map(range => <button key={range} type="button" onClick={() => setTelegramStatsRange(range)} className={`h-8 rounded-md text-[10px] font-medium ${telegramStatsRange === range ? "bg-[#3f8cff]/20 text-[#b7d2ff]" : "text-slate-500"}`}>{range === "day" ? "День" : range === "month" ? "Месяц" : "Всё время"}</button>)}</div><TelegramAnalyticsChart title="Рост" graph={target.snapshot.history.graphs?.growth} range={telegramStatsRange} /><TelegramAnalyticsChart title="Подписки / отписки" graph={target.snapshot.history.graphs?.subscriptions} range={telegramStatsRange} accent="#f26667" /><TelegramAnalyticsChart title="Охват и взаимодействия" graph={target.snapshot.history.graphs?.reach} range={telegramStatsRange} accent="#b38cff" /><TelegramAnalyticsChart title="Активные часы" graph={target.snapshot.history.graphs?.activeHours} range={telegramStatsRange} accent="#f5b84b" /><TelegramAnalyticsChart title="Источники просмотров" graph={target.snapshot.history.graphs?.viewsBySource ?? target.snapshot.history.graphs?.followersBySource} range={telegramStatsRange} accent="#57d5a2" /></> : target.snapshot ? <p className="text-[10px] leading-4 text-slate-400">Есть старая короткая выгрузка. Нажми «Обновить», чтобы загрузить полный набор графиков Telegram.</p> : <p className="text-[10px] leading-4 text-slate-500">{target.availability === "unavailable" ? "Telegram пока не отдал статистику этому аккаунту." : "Канал разрешён. Нажми «Обновить», чтобы получить историю."}</p>}</div>)}</div>
