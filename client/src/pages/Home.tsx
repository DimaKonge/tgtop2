...
export default function Home() {
  ...
  return <div className="tg-shell min-h-screen bg-[#0b0f14] text-slate-100">
    ...
    <main className="mx-auto max-w-3xl px-4 pb-28 pt-5">
      {page === "top" && <section className="space-y-4"><div className="flex items-center justify-between"><h1 className="text-2xl font-semibold">Global</h1><Button onClick={() => setFiltersOpen(true)} variant="outline" className="border-white/10 bg-[#111720] text-slate-200"><Filter className="mr-2 h-4 w-4" />Фильтр</Button></div><div className="space-y-3"><GroupCard group={leadSlot.group} variant="lead" onClick={() => leadSlot.group ? openGroup(leadSlot.group.id) : openMine(leadSlot)} /><div className="grid grid-cols-2 gap-3">{secondTier.map(slot => <GroupCard key={slot.slotNumber} group={slot.group} variant="secondary" onClick={() => slot.group ? openGroup(slot.group.id) : openMine(slot)} />)}</div><div className="grid grid-cols-4 gap-2">{thirdTier.map(slot => <GroupCard key={slot.slotNumber} group={slot.group} variant="compact" onClick={() => slot.group ? openGroup(slot.group.id) : openMine(slot)} />)}</div></div><section className="pt-2"><div className="space-y-2">{generalList.map(group => <GroupCard key={group.id} group={group} variant="list" onClick={() => openGroup(group.id)} />)}{generalList.length === 0 && <div className="rounded-2xl border border-dashed border-white/12 bg-[#111720] p-6 text-center"><p className="text-sm font-medium text-slate-300">В TG TOP пока нет площадок</p><p className="mt-1 text-xs text-slate-500">Добавьте первую группу через личную папку.</p></div>}</div></section></section>}
