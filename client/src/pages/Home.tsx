...
import { WalletNftPickerModal } from "@/components/nft/WalletNftPickerModal";
...
function WalletNftCard({ item, language, onList }: { item: WalletNft; language: Language; onList?: (item: WalletNft) => void }) {
  ...
  const handleOpenListing = () => {
    if (onList) {
      ...
      window.dispatchEvent(new CustomEvent("tgtop:list-wallet-nft", { detail: item }));
    }
  };
  ...
  return (
    <article
      ...
      className="group relative cursor-pointer overflow-hidden rounded-xl border border-white/9 bg-[#111720] p-2.5 transition-colors hover:border-[#3f8cff]/35 hover:bg-[#151d29] active:scale-[0.99]"
    >
      <div className="relative aspect-square overflow-hidden rounded-lg border border-white/8 bg-[#1b2430]">
        {imageUrl && !imageFailed ? (
          item.mediaKind === "video" && imageIndex === 0 ? (
            <video
              src={imageUrl}
              autoPlay
              muted
          ...
          <span className="grid h-full w-full place-items-center bg-[radial-gradient(circle_at_30%_20%,rgba(84,143,255,.32),transparent 42%),#152131] text-lg font-semibold text-[#aacaff]">
            {item.name.slice(0, 1).toUpperCase()}
          </span>
        )}
        <span
          className={`absolute left-1.5 top-1.5 rounded-md border px-1.5 py-1 text-[8px] font-semibold backdrop-blur-sm ${categoryClass[item.category]}`}
        >
      ...
      <small className="mt-0.5 block truncate text-[9px] text-slate-500">
        {item.collectionName ?? categoryLabel[item.category]}
      </small>
      <small className="mt-1 block truncate font-mono text-[8px] text-slate-600">
        {shortAddress}
      </small>
      <button
        type="button"
        ...
        className="mt-2 flex w-full items-center justify-center gap-1 rounded-lg border border-[#3f8cff]/35 bg-[#3f8cff]/10 py-1.5 text-[10px] font-semibold text-[#a6c8ff] transition-colors hover:bg-[#3f8cff]/20 active:scale-[0.98]"
      >
        <Tag className="h-3 w-3" />
        <span>{language === "en" ? "List / Rent" : "Настроить листинг"}</span>
      </button>
    </article>
  );
}
...
function SafeAvatar({ src, fallback, className = "", userId }: { src?: string | null; fallback: ReactNode; className?: string; userId?: string | null }) {
  ...
  useEffect(() => setProxyFailed(false), [src, userId]);

  // If the primary source fails, and we have a Telegram User ID, try the server-side proxy.
  const proxySrc = (failed && !proxyFailed && userId && src !== `/api/telegram-user-avatar/${userId}`)
    ? `/api/telegram-user-avatar/${userId}`
    : null;

  const currentSrc = proxySrc || (!failed ? src : null);
...
export default function Home({ onReady }: { onReady?: () => void }) {
  ...
  const [selectedWalletNftToList, setSelectedWalletNftToList] = useState<WalletNft | null>(null);
  const [nftListingSheetOpen, setNftListingSheetOpen] = useState(false);
  const [walletNftPickerOpen, setWalletNftPickerOpen] = useState(false);
  const [selectedMarketNftToRent, setSelectedMarketNftToRent] = useState<Nft | null>(null);
  const [nftMarketRentalSheetOpen, setNftMarketRentalSheetOpen] = useState(false);
  const [selectedMarketNftToBuy, setSelectedMarketNftToBuy] = useState<Nft | null>(null);
  const [nftMarketBuySheetOpen, setNftMarketBuySheetOpen] = useState(false);

  useEffect(() => {
    const handleNftListEvent = (e: Event) => {
      const detail = (e as CustomEvent<WalletNft>).detail;
      if (detail) {
        setSelectedWalletNftToList(detail);
        setNftListingSheetOpen(true);
      }
    };
    window.addEventListener("tgtop:list-wallet-nft", handleNftListEvent);
    return () => window.removeEventListener("tgtop:list-wallet-nft", handleNftListEvent);
  }, []);
  ...
  const walletNftsQuery = trpc.tgTop.getWalletNfts.useQuery(
    ...
    {
      enabled: isAuthenticated && Boolean(safeWalletAddress) && walletConnectionRestored && ((page === "mine" && workspaceSection === "nft") || (page === "top" && topSection === "nft") || nftListingSheetOpen),
      staleTime: 45_000,
      retry: 1,
  ...
  const [userAvatarStage, setUserAvatarStage] = useState(0);

  const displayUserAvatar = useMemo(() => {
    const directUrl = telegramAvatar
    ...
    const sources = Array.from(
      new Set([directUrl, proxyUrl, usernameUrl, user?.avatarUrl].filter((s): s is string => Boolean(s)))
    );

    if (userAvatarStage < sources.length) {
      return sources[userAvatarStage];
    }
    return null;
  ...
  const renderUserAvatar = (size: "sm" | "md") => {
    ...
    const hasPhoto = Boolean(displayUserAvatar);

    return (
      <span
        className={`grid ${sizeClasses} shrink-0 place-items-center overflow-hidden rounded-full border border-white/15 bg-[#17212b] font-bold text-white shadow-sm ring-1 ring-white/10`}
      >
  ...
  return (
    <div className="tg-shell min-h-screen bg-[#0b0f14] text-slate-100">
          ...
          <section className="space-y-2">
            <div className="flex flex-wrap items-center gap-2">
              ...
              {([
                ...
                <button key={value} type="button" onClick={() => selectTopSection(value)} className={`tg-top-tab h-8 rounded-lg text-[10px] font-semibold transition-all ${topSection === value ? "tg-top-tab-active bg-[#3f8cff] text-white shadow-md shadow-[#3f8cff]/25 font-bold" : "text-slate-400 hover:bg-white/5 hover:text-slate-200"}`}>{label}</button>
              ))}
            </div>
            ...
            {topSection === "bots" ? (
              ...
              <section className="space-y-2 pt-1">
                <div className="flex items-center justify-between gap-2 px-1">
                  ...
                  <div className="flex items-center gap-1.5">
                    <button
                      ...
                      onClick={() => {
                        if (!isAuthenticated) {
                          toast.error(tx("Войдите в аккаунт, чтобы выставить NFT", "Sign in to list an NFT"));
                          return;
                        }
                        setWalletNftPickerOpen(true);
                      }}
                      className="flex items-center gap-1 rounded-lg bg-[#3f8cff] px-2.5 py-1.5 text-[10px] font-semibold text-white shadow-sm transition-colors hover:bg-[#3478db] active:scale-[0.98]"
                    >
                      <Plus className="h-3 w-3" />
                      <span>{tx("Залистить NFT", "List NFT")}</span>
                    </button>
                    {isAuthenticated && (
                ...
                {safeWalletAddress && walletNfts.length > 0 && (
                  <div className="rounded-xl border border-[#3f8cff]/25 bg-[#3f8cff]/[0.06] p-2.5">
                    <div className="flex items-center justify-between gap-2">
                      <span className="flex items-center gap-1.5">
                        <Tag className="h-3.5 w-3.5 text-[#3f8cff]" />
                        <span className="text-xs font-medium text-slate-200">
                          {tx("Ваши NFT в кошельке", "Your wallet NFTs")} ({walletNfts.length})
                        </span>
                      </span>
                      <button
                        type="button"
                        onClick={() => setWalletNftPickerOpen(true)}
                        className="text-[10px] font-semibold text-[#8fb9ff] hover:underline"
                      >
                        {tx("Все в кошельке →", "View all →")}
                      </button>
                    </div>
                    <div className="mt-2 flex gap-2 overflow-x-auto pb-1 [scrollbar-width:none] [&::-webkit-scrollbar]:hidden">
                      {walletNfts.slice(0, 6).map((item) => (
                        <div
                          key={item.address}
                          onClick={() => {
                            setSelectedWalletNftToList(item);
                            setNftListingSheetOpen(true);
                          }}
                          className="flex w-24 shrink-0 cursor-pointer flex-col items-center rounded-lg border border-white/8 bg-[#10161f] p-1.5 transition-all hover:border-[#3f8cff]/40 hover:bg-[#16202d] active:scale-[0.97]"
                        >
                          <div className="relative aspect-square w-full overflow-hidden rounded-md border border-white/8 bg-[#182230]">
                            {item.imageUrl ? (
                              <img src={item.imageUrl} alt="" className="h-full w-full object-cover" referrerPolicy="no-referrer" />
                            ) : (
                              <span className="grid h-full w-full place-items-center text-xs font-bold text-[#8fb9ff]">
                                {item.name.slice(0, 1).toUpperCase()}
                              </span>
                            )}
                          </div>
                          <span className="mt-1 block w-full truncate text-center text-[10px] font-semibold text-slate-100">
                            {item.name}
                          </span>
                          <span className="mt-0.5 inline-flex items-center gap-0.5 text-[9px] font-medium text-[#3f8cff]">
                ...
                {safeWalletAddress && walletNfts.length === 0 && (
                  <div className="flex items-center justify-between gap-3 rounded-xl border border-white/8 bg-[#111720] p-3">
                    <span className="min-w-0">
                      <b className="block text-xs text-slate-200">{tx("Кошелёк подключён", "Wallet connected")}</b>
                      <small className="block text-[10px] text-slate-400">
                        {walletNftsQuery.isLoading
                          ? tx("Ищем ваши NFT в сети TON…", "Searching your NFTs on TON…")
                          : tx("NFT пока не найдены. Можно указать контракт вручную", "No NFTs found yet. You can specify contract manually")}
                      </small>
                    </span>
                    <button
                      type="button"
                      onClick={() => setWalletNftPickerOpen(true)}
                      className="shrink-0 rounded-lg bg-[#3f8cff] px-2.5 py-1.5 text-[10px] font-semibold text-white transition-colors hover:bg-[#3478db]"
                    >
                      {tx("Выставить NFT", "List NFT")}
                    </button>
                  </div>
                )}
                {!safeWalletAddress && isAuthenticated && (
                  <div className="flex items-center justify-between gap-3 rounded-xl border border-white/8 bg-[#111720] p-3">
                    <span className="min-w-0">
                      <b className="block text-xs text-slate-200">{tx("Хотите выставить свои NFT?", "Want to list your NFTs?")}</b>
                      <small className="block text-[10px] text-slate-400">{tx("Подключите TON-кошелёк, чтобы загрузить ваши гифты и юзернеймы", "Connect your TON wallet to load gifts and usernames")}</small>
                    </span>
                    <button
                      type="button"
                      onClick={() => setWalletNftPickerOpen(true)}
                  ...
                  <div className="space-y-2">
                    {visibleNfts.map(nft => (
                      <NftCard
                        ...
                        language={language}
                        onRent={item => {
                          setSelectedMarketNftToRent(item);
                          setNftMarketRentalSheetOpen(true);
                        }}
                        onBuy={item => {
                          setSelectedMarketNftToBuy(item);
                          setNftMarketBuySheetOpen(true);
                        }}
                      />
                    ))}
                  </div>
          ...
          <section className={`space-y-4 ${myGroupsSelectionMode ? "pb-[12rem]" : ""}`}>
            ...
            <div className="grid grid-cols-3 overflow-hidden rounded-xl border border-white/8 bg-black/15 p-0.5">
              {([
                ...
                return <button key={value} type="button" onClick={() => { setWorkspaceSection(value); if (value !== "communities") exitMyGroupsSelection(); }} className={`flex h-8 min-w-0 items-center justify-center gap-1 rounded-lg px-1 text-[10px] font-medium transition-all ${active ? "bg-[#3f8cff] text-white shadow-md shadow-[#3f8cff]/25 font-bold" : "text-slate-400 hover:text-slate-200"}`}><Icon className="h-3.5 w-3.5 shrink-0" /><span className="truncate">{label}</span></button>;
              })}
            </div>
      ...
      <NftListingSheet
        nft={selectedWalletNftToList}
        open={nftListingSheetOpen}
        onOpenChange={setNftListingSheetOpen}
        language={language}
        walletAddress={safeWalletAddress}
      />
      <WalletNftPickerModal
        open={walletNftPickerOpen}
        onOpenChange={setWalletNftPickerOpen}
        walletAddress={safeWalletAddress}
        language={language}
        onSelectNft={(nft) => {
          setSelectedWalletNftToList(nft);
          setNftListingSheetOpen(true);
        }}
        onConnectWallet={openTonWalletForCurrentUser}
      />
      <NftRentalSheet
        nft={selectedMarketNftToRent}
        open={nftMarketRentalSheetOpen}
        onOpenChange={setNftMarketRentalSheetOpen}
        language={language}
      />
      <NftBuySheet
        nft={selectedMarketNftToBuy}
        open={nftMarketBuySheetOpen}
        onOpenChange={setNftMarketBuySheetOpen}
        language={language}
      />
    </div>
  );
