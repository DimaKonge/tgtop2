                    ...
                    <SheetContent side="bottom" className="max-h-[88dvh] overflow-y-auto rounded-t-[28px] border-white/10 bg-[#10161f] text-slate-100">
                      ...
                      <div className="space-y-4 px-5 pb-[calc(1.25rem+env(safe-area-inset-bottom))]">
                        {tonWithdrawalFlow === "form" && <>
                          ...
                          <label className="block"><span className="mb-2 block text-center text-[11px] font-medium uppercase tracking-[0.12em] text-slate-400">{tx("Сумма вывода · GRAM", "Withdrawal amount · GRAM")}</span><Input value={tonWithdrawalAmount} inputMode="decimal" onChange={event => { const value = event.target.value.replace(",", "."); if (/^\d*(\.\d{0,9})?$/.test(value)) setTonWithdrawalAmount(value); }} className="h-16 rounded-2xl border-white/10 bg-white/[0.045] px-4 text-center text-2xl font-semibold tracking-tight text-emerald-100" placeholder="0.1" /><small className="mt-2 block text-center text-[11px] text-slate-500">{tx("Минимум 0.1 GRAM. Комиссия сети вычитается из этой суммы.", "Minimum 0.1 GRAM. Network fee is deducted from this amount.")}</small></label>
                          <label className="block"><span className="mb-2 block text-center text-[11px] font-medium uppercase tracking-[0.12em] text-slate-400">{tx("Адрес получателя", "Recipient address")}</span><Input value={tonWithdrawalAddress} onChange={event => setTonWithdrawalAddress(event.target.value.trim())} autoCapitalize="none" autoCorrect="off" spellCheck={false} className="h-16 rounded-2xl border-white/10 bg-white/[0.045] px-4 text-center font-mono text-sm text-slate-100" placeholder="UQ… или EQ…" /><small className="mt-2 block text-center text-[11px] leading-4 text-slate-500">{tonWithdrawalAddress === tonWithdrawalDefaultRecipient && tonWithdrawalDefaultRecipient ? tx("Подставлен привязанный адрес из профиля. Его можно изменить.", "The linked profile address is filled in. You can change it.") : tx("Можно указать другой mainnet-адрес. Новый адрес проходит ручную проверку.", "You can enter another mainnet address. A new address is held for manual review.")}</small></label>
                          <button type="button" disabled={quoteTonWithdrawalMutation.isPending || !tonWithdrawalAddress || !tonWithdrawalAmount} onClick={() => void prepareTonWithdrawal()} className="flex h-14 w-full items-center justify-center gap-2 rounded-2xl bg-emerald-500 px-4 text-base font-semibold text-slate-950 transition-colors hover:bg-emerald-400 disabled:opacity-50"><Send className="h-5 w-5" />{quoteTonWithdrawalMutation.isPending ? tx("Считаем комиссию…", "Calculating fee…") : tx("Продолжить", "Continue")}</button>
                        </>}
